...
local function sharedEnvironment()
    ...
    check(ok and type(env) == "table", "getgenv must return an environment table")
    return env
end
local function hostCall(fn, label, ...)
    check(type(fn) == "function", "host does not provide " .. label)
    local ok, result = pcall(fn, ...)
    check(ok, label .. " failed: " .. tostring(result))
    return result
end
local function isInstance(value)
    return type(typeof) == "function" and typeof(value) == "Instance"
end
local function pathParts(path)
    check(type(path) == "string" and not path:find("[%z\1-\31\127]"), "input must be an instance path without control characters")
    local parts, pos, size = {}, 1, #path
    local function space()
        while pos <= size and path:sub(pos, pos):match("%s") do pos = pos + 1 end
    end
    local function name()
        space()
        local first, last = path:find("[A-Za-z_][A-Za-z0-9_]*", pos)
        check(first == pos, "expected instance name at path position " .. pos)
        local value = path:sub(first, last); pos = last + 1
        return value
    end
    local function quoted()
        space(); local delimiter = path:sub(pos, pos); pos = pos + 1
        check(delimiter == '"' or delimiter == "'", "path keys and service names require a quoted string literal")
        local chars, closed = {}, false
        while pos <= size do
            local c = path:sub(pos, pos); pos = pos + 1
            if c == delimiter then closed = true; break end
            if c == "\\" then
                c = path:sub(pos, pos); pos = pos + 1
                check(c == "\\" or c == '"' or c == "'", "unsupported escape in instance path")
            end
            chars[#chars + 1] = c
        end
        check(closed, "unterminated quoted path string")
        return concat(chars)
    end
    local rootName = name()
    parts[1] = { kind = "member", name = rootName }
    while true do
        space(); if pos > size then break end
        local symbol = path:sub(pos, pos); pos = pos + 1
        if symbol == "." then
            parts[#parts + 1] = { kind = "member", name = name() }
        elseif symbol == "[" then
            local key = quoted()
            space(); check(path:sub(pos, pos) == "]", "expected closing path bracket"); pos = pos + 1
            parts[#parts + 1] = { kind = "member", name = key }
        elseif symbol == ":" then
            local method = name()
            check(method == "GetService" and #parts == 1 and (rootName == "game" or rootName == "Game"), "only game:GetService(quoted service name) is supported in instance paths")
            space(); check(path:sub(pos, pos) == "(", "expected '(' after GetService"); pos = pos + 1
            local service = quoted()
            space(); check(path:sub(pos, pos) == ")", "GetService requires exactly one quoted service name"); pos = pos + 1
            parts[#parts + 1] = { kind = "service", name = service }
        else fail("instance paths accept names, dots, quoted brackets and game:GetService(quoted service name); not arbitrary expressions") end
    end
    return parts
end
function D.resolve(target)
    if isInstance(target) then return target end
    local parts = pathParts(target)
    check(isInstance(game), "host does not provide Roblox game")
    local current, first = game, 1
    if parts[1].name == "game" or parts[1].name == "Game" then first = 2 end
    for j = first, #parts do
        local part = parts[j]
        local name = part.name
        if part.kind == "service" then
            current = hostCall(game.GetService, "GetService", game, name)
            check(isInstance(current), "GetService did not return an Instance: " .. quote(name))
        elseif current == game and (name == "workspace" or name == "Workspace") then
            current = hostCall(game.GetService, "GetService", game, "Workspace")
            check(isInstance(current), "Workspace service is not available")
        else
            local ok, member = pcall(function() return current[name] end)
            check(ok, "cannot read instance member " .. quote(name) .. ": " .. tostring(member))
            check(member ~= nil, "instance member " .. quote(name) .. " is nil (not available yet)")
            check(isInstance(member), "instance member " .. quote(name) .. " is not an Instance")
            current = member
        end
    end
    return current
...
local function folderExists(isFolder, path)
    local ok, value = pcall(isFolder, path)
    return ok and value == true
end
local function fileExists(isFile, path)
    local ok, value = pcall(isFile, path)
    return ok and value == true
end
local function ensureFolder(isFolder, makeFolder, path)
    local parts = {}
    for part in path:gmatch("[^/]+") do parts[#parts + 1] = part end
    local current = ""
    for _, part in ipairs(parts) do
        current = current == "" and part or (current .. "/" .. part)
        if not folderExists(isFolder, current) then
            local ok, err = pcall(makeFolder, current)
            check(ok, "makefolder failed for " .. quote(current) .. ": " .. tostring(err))
        end
    end
end
local function uniquePath(isFile, path)
    if not fileExists(isFile, path) then return path end
    local base, ext = path:match("^(.*)%.([^./]+)$")
    if not base then base, ext = path, "lua" end
    for n = 1, 100000 do
        local candidate = base .. "_" .. n .. "." .. ext
        if not fileExists(isFile, candidate) then return candidate end
    end
    fail("cannot find a free file name for " .. quote(path))
end
local function safeSegment(name)
    check(type(name) == "string", "instance has no string Name")
    name = name:gsub('[%z\1-\31\127/\\:<>"|?*]', "_"):gsub("^[. ]+", "_"):gsub("[. ]+$", "_")
    if name == "" then name = "Instance" end
    local base = name:match("^([^.]*)"):lower()
    if base == "con" or base == "prn" or base == "aux" or base == "nul" or base:match("^com[1-9]$") or base:match("^lpt[1-9]$") then name = "_" .. name end
    return name
end
local function gameName()
    ...
    if okPlace and type(placeId) == "number" then
        local okService, marketplace = pcall(function() return game:GetService("MarketplaceService") end)
        if okService and marketplace then
            local okInfo, info = pcall(function() return marketplace:GetProductInfo(placeId) end)
            if okInfo and type(info) == "table" and type(info.Name) == "string" and info.Name ~= "" then return info.Name end
        end
...
local function childrenOf(instance)
    local ok, list = pcall(function() return instance:GetChildren() end)
    if ok and type(list) == "table" then return list end
    return {}
end
local function isScriptInstance(instance)
    local ok, value = pcall(function() return instance:IsA("LuaSourceContainer") end)
    return ok and value == true
end
local function relativePath(instance, root)
    local parts, current, guard = {}, instance, 0
    while current and current ~= root do
        guard = guard + 1
        if guard > 512 then return nil end
        parts[#parts + 1] = safeSegment(current.Name)
...
local function collectScripts(root)
    ...
    local function record(instance)
        local rel = relativePath(instance, root)
        if rel and rel ~= "" then list[#list + 1] = { instance = instance, path = rel .. ".lua" } end
    end
    local ok, descendants = pcall(function() return root:GetDescendants() end)
    if ok and type(descendants) == "table" then
        for _, child in ipairs(descendants) do
            if isScriptInstance(child) then record(child) end
        end
    else
        local stack = { root }
        while #stack > 0 do
            local current = table.remove(stack)
            for _, child in ipairs(childrenOf(current)) do
                if isScriptInstance(child) then record(child) end
                stack[#stack + 1] = child
            end
        end
...
local function resolveRoot(path)
    local ok, result = pcall(D.resolve, path)
    if ok then return result end
    return nil, tostring(result)
end
function D.decompileAll(config)
    check(type(config) == "table", "decompileAll expects a configuration table")
    check(config.quiet == nil or type(config.quiet) == "boolean", "quiet must be boolean")
    local env = sharedEnvironment()
    local extract = getscriptbytecode or rawget(env, "getscriptbytecode")
    local write = writefile or rawget(env, "writefile")
    local read = readfile or rawget(env, "readfile")
    local isFile = isfile or rawget(env, "isfile")
    local makeFolder = makefolder or rawget(env, "makefolder")
    local isFolder = isfolder or rawget(env, "isfolder")
    check(type(extract) == "function", "host does not provide getscriptbytecode; an instance bytecode extractor is required")
    check(type(write) == "function" and type(read) == "function" and type(isFile) == "function", "batch mode requires writefile, readfile and isfile")
    check(type(makeFolder) == "function" and type(isFolder) == "function", "batch mode requires makefolder and isfolder")
    local options = copy(config.options or {})
    if options.vector_constructor == nil and type(Vector3) == "table" and type(Vector3.new) == "function" then options.vector_constructor = "Vector3.new" end
    local rootFolder = config.folder
    if rootFolder == nil then rootFolder = safeSegment(gameName() or "decompile") end
    check(type(rootFolder) == "string" and rootFolder ~= "" and not rootFolder:find("[%z\1-\31\127]"), "folder must be a non-empty relative path")
    rootFolder = rootFolder:gsub("/+$", "")
    ensureFolder(isFolder, makeFolder, rootFolder)
    local roots = {
        { label = "PlayerScripts", path = "game:GetService('Players').LocalPlayer.PlayerScripts" },
        { label = "ReplicatedStorage", path = "game:GetService('ReplicatedStorage')" },
    }
    local report = { folder = rootFolder, roots = {}, scripts = 0, saved = 0, failed = 0, files = {}, errors = {} }
    for _, root in ipairs(roots) do
        local entry = { label = root.label, scripts = 0, saved = 0, failed = 0 }
        report.roots[#report.roots + 1] = entry
        local instance, reason = resolveRoot(root.path)
        if not instance then
            entry.error = "could not resolve " .. root.path .. (reason and (" (" .. reason .. ")") or "")
        else
            local base = rootFolder .. "/" .. root.label
            ensureFolder(isFolder, makeFolder, base)
            local scripts = collectScripts(instance)
            entry.scripts = #scripts
            report.scripts = report.scripts + #scripts
            for _, item in ipairs(scripts) do
                local ok, err = pcall(function()
                    local bytes = hostCall(extract, "getscriptbytecode", item.instance)
                    check(type(bytes) == "string" and #bytes > 0, "getscriptbytecode returned no bytecode")
                    local source = D.decompile(bytes, options)
                    local relative = item.path:match("^(.*)/[^/]*$")
                    local dir = base
                    if relative and relative ~= "" then
                        dir = base .. "/" .. relative
                        ensureFolder(isFolder, makeFolder, dir)
                    end
                    local leaf = item.path:match("[^/]*$")
                    local target = uniquePath(isFile, dir .. "/" .. leaf)
                    check(hostCall(write, "writefile", target, source) ~= false, "writefile reported failure")
                    check(hostCall(read, "readfile", target) == source, "output verification failed")
                    entry.saved = entry.saved + 1
                    report.saved = report.saved + 1
                    report.files[#report.files + 1] = target
                end)
                if not ok then
                    entry.failed = entry.failed + 1
                    report.failed = report.failed + 1
                    report.errors[#report.errors + 1] = item.path .. ": " .. tostring(err)
                end
            end
        end
    end
    if not config.quiet then
        local notify = type(warn) == "function" and warn or print
        if type(notify) == "function" then
            notify("luau-decompiler: " .. quote(rootFolder) .. " -> " .. report.saved .. " saved, " .. report.failed .. " failed, " .. report.scripts .. " found")
            for _, root in ipairs(report.roots) do
                local summary = "luau-decompiler: " .. root.label .. ": " .. root.saved .. "/" .. root.scripts .. " decompiled"
                if root.error then summary = summary .. " (" .. root.error .. ")" end
                notify(summary)
            end
            for _, err in ipairs(report.errors) do notify("luau-decompiler: failed " .. err) end
        end
    end
    return report
end
...
local function decompileScript(scr)
	local okb, bytes = pcall(function() return getscriptbytecode(scr) end)
	if not okb or type(bytes) ~= "string" or #bytes == 0 then
		return nil, "failed to extract bytecode"
	end
	local oks, source = pcall(function()
		return DexDecompiler.decompile(bytes, {
			header = false,
			allow_trailing = true,
			vector_constructor = "Vector3.new",
		})
	end)
	if not oks or type(source) ~= "string" then
		...
		local function main()
			...
			Explorer.ShowRightClick = function(MousePos)
				...
				if presentClasses["LuaSourceContainer"] then
					context:AddRegistered("VIEW_SCRIPT", not presentClasses.isViableDecompileScript or type(getscriptbytecode) ~= "function")
					context:AddRegistered("SAVE_SCRIPT", not presentClasses.isViableDecompileScript or type(getscriptbytecode) ~= "function" or env.writefile == nil)
				...
				context:Register("SAVE_SCRIPT",{Name = "Save Script", IconMap = Explorer.MiscIcons, Icon = "Save", OnClick = function()
					for _, v in next, selection.List do
						if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
							local source, derr = decompileScript(v.Obj)
							if not source then source = ("-- DEX - %s failed to decompile %s\n-- %s"):format(env.executor, v.Obj.ClassName, tostring(derr)) end
	...
	ScriptViewer = function()
		...
		local function main()
			...
			ScriptViewer.ViewScript = function(scr)
				local source, derr = decompileScript(scr)
				if not source then source, PreviousScr = ("-- DEX - %s failed to decompile %s\n-- %s"):format(env.executor, scr.ClassName, tostring(derr)), nil else PreviousScr = scr end
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="Dex Fixed",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed by Moon & 猪咪人反编译修复",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
